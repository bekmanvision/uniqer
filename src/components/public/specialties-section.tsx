...
export function SpecialtiesSection({ majors, topMajors, label = 'Специальности бакалавриата' }: SpecialtiesSectionProps) {
    ...
    <Card>
      <CardContent className="p-6 lg:p-8">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
            ...
            <span
              ...
              className={
                topMajors && topMajors.includes(major)
                  ? 'rounded-full bg-yellow-50 px-3 py-1.5 text-sm font-medium text-yellow-800'
                  : 'rounded-full bg-gray-100 px-3 py-1.5 text-sm text-gray-700'
              }
            >
              {major}
            </span>
          ...
          {total > 5 && !isOpen && (
            <span className="rounded-full bg-gray-50 px-3 py-1.5 text-sm text-gray-400">
              +{total - 5}
            </span>
          )}
        </div>
        ...
        {total > 5 && !isOpen && (
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-blue-50 px-4 py-2.5 text-sm font-medium text-blue-700 transition-colors hover:bg-blue-100"
          >
            <GraduationCap className="h-4 w-4" />
            Все специальности ({total})
            ...
            <div className="flex flex-wrap gap-2">
              {filtered.map((major, i) => (
                <span
                  ...
                  className={
                    topMajors && topMajors.includes(major)
                      ? 'rounded-full bg-yellow-50 px-3 py-1.5 text-sm font-medium text-yellow-800 border border-yellow-100'
                      : 'rounded-full bg-white px-3 py-1.5 text-sm text-gray-700 border border-gray-200'
                  }
                >
                  {major}
                </span>
