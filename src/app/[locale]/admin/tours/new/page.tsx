...
export default function NewTourPage() {
  ...
  return (
    <div>
      <Link href="/admin/tours" className="mb-4 inline-flex items-center text-sm text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400">
        <ArrowLeft className="mr-1 h-4 w-4" />
        {t('backToTours')}
      </Link>

      <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t('newTour')}</h1>
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            />
            <div>
              <label className="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">{t('forGrades')}</label>
              <div className="flex flex-wrap gap-4">
                {['8', '9', '10', '11', '12'].map((g) => (
                  <label key={g} className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-sm text-gray-700 dark:text-gray-300">{t('gradeLabel', { g })}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-6">
              ...
              <label className="flex items-center gap-2">
                <input
                  ...
                  className="rounded border-gray-300 dark:border-gray-700"
                />
                <span className="text-sm text-gray-700 dark:text-gray-300">{t('featuredTour')}</span>
              </label>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader><CardTitle>{t('universities')}</CardTitle></CardHeader>
          <CardContent>
            {universities.length === 0 ? (
              <p className="text-sm text-gray-500 dark:text-gray-400">{t('addUnisFirst')}</p>
            ) : (
              <div className="space-y-4">
                {Object.entries(
                  ...
                  <div key={city}>
                    <h4 className="mb-2 text-sm font-semibold text-gray-900 dark:text-gray-100">{city}</h4>
                      ...
                      {unis.map((uni) => (
                        <label key={uni.id} className="flex items-center gap-2">
                          <input
                            ...
                            className="rounded border-gray-300 dark:border-gray-700"
                          />
                          <span className="text-sm text-gray-700 dark:text-gray-300">{uni.name}</span>
                        </label>
                      ))}
                    </div>
                  </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle>{t('programByDays')}</CardTitle>
              <Button type="button" variant="outline" size="sm" onClick={addDay}>
                <Plus className="mr-1 h-4 w-4" /> {t('dayBtn')}
              </Button>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            {formData.program.map((day, dayIndex) => (
              <div key={dayIndex} className="rounded-lg border dark:border-gray-700 p-4">
                <div className="mb-3 flex items-center justify-between">
                  <h4 className="font-medium">{t('dayLabel', { day: day.day })}</h4>
                ...
                <Input
                  placeholder={t('dayNamePlaceholderNew')}
                  value={day.title}
                ...
                <div className="space-y-2">
                  ...
                  <Button type="button" variant="ghost" size="sm" onClick={() => addActivity(dayIndex)}>
                    <Plus className="mr-1 h-4 w-4" /> {t('activityBtn')}
                  </Button>
                </div>
              </div>
            ))}
        ...
        <div className="flex justify-end gap-4">
          ...
          <Button type="submit" loading={loading}>{t('createTour')}</Button>
        </div>
      </form>
    </div>
  )
