...
export default function InternationalUniversitiesPage() {
  ...
  const deleteUniversity = async (id: string) => {
    if (!confirm(t('deleteInternationalUniConfirm'))) return
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t('internationalUniversities')}</h1>
          <p className="text-sm text-gray-600 dark:text-gray-400">{t('totalUniCount', { count: universities.length })}</p>
        </div>
            ...
            <Card key={university.id}>
              <CardContent className="p-4">
                ...
                {university.worldRanking && (
                  <div className="mb-2">
                    <span className="inline-flex items-center rounded-full bg-blue-50 dark:bg-blue-900/30 px-2 py-1 text-xs font-medium text-blue-700 dark:text-blue-400">
                      {t('rankingLabel', { ranking: university.worldRanking })}
                    </span>
                  </div>
                )}
                ...
                {university.topMajors.length > 0 && (
                  <div className="mb-2">
                    <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">{t('topMajors')}</p>
                    <div className="flex flex-wrap gap-1">
                      {university.topMajors.slice(0, 3).map((major, idx) => (
                        <span key={idx} className="inline-flex items-center rounded bg-gray-100 dark:bg-gray-800 px-2 py-0.5 text-xs text-gray-700 dark:text-gray-300">
                          {major}
                        </span>
                      ))}
                    </div>
      ...
      <div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-xl bg-white dark:bg-gray-900 shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="hasDormitory" className="text-sm text-gray-700 dark:text-gray-300">
              {t('hasDormitory')}
            </label>
          </div>
