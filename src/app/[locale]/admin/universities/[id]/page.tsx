...
export default function EditUniversityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/universities/${id}`)
      ...
      .then(data => {
        if (data.success) {
          const uni = data.data as University
          setFormData({
            name: uni.name,
            city: uni.city,
            type: uni.type,
            description: uni.description,
            majors: uni.majors.length ? uni.majors : [''],
            grants: uni.grants,
            paid: uni.paid,
            logo: uni.logo || '',
            images: uni.images.length ? uni.images : [''],
            website: uni.website || '',
          })
        }
        setFetching(false)
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t('editUni')}</h1>
